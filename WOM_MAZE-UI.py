...
class PathVisualizerApp(ctk.CTk):
    ...
    def dfs_generator(self, grid, start, goal):
        visited = set()
        stack = [start]
        parent = {start: None}
        while stack:
    ...
    def dijkstra_generator(self, grid, start, goal):
        import heapq
        dist = {start: 0}
        parent = {}
        visited = set()
    ...
    def astar_generator(self, grid, start, goal, heuristic):
        import heapq
        g_score = {start: 0}
        # Weighted A*: f = g + w * h
        w = getattr(self, 'weight', 1.0)
        f_score = {start: w * logic.get_heuristic(start, goal, heuristic)}
        ...
        while open_set:
            ...
            for v, cost in logic.get_neighbors_cost(u, len(grid)):
                ...
                f_score[v] = tentative_g + w * logic.get_heuristic(v, goal, heuristic)
    ...
    def start_pathfinding(self):
        ...
        try:
            ...
            if algo == 'BFS':
                self.step_gen = self.bfs_generator(grid, self.start, self.end)
            elif algo == 'DFS':
                self.step_gen = self.dfs_generator(grid, self.start, self.end)
            elif algo == 'Dijkstra':
                self.step_gen = self.dijkstra_generator(grid, self.start, self.end)
            else:
